...
import {
  Send, Paperclip, Image as ImageIcon, Mic, Sparkles,
  ChevronDown, Share2, MoreVertical, X, Loader2,
} from 'lucide-react'
...
export default function Chat() {
  ...
  return (
    ...
    <div className="flex-1 flex flex-col min-w-0 min-h-0">
      {/* Top bar */}
      <div className="flex items-center justify-end gap-3 px-4 sm:px-6 py-3 pl-16 md:pl-6 border-b border-border">
        <div className="flex items-center gap-1 flex-shrink-0">
          <button className="focus-ring w-9 h-9 flex items-center justify-center rounded-lg text-on-muted hover:bg-surface-4 hover:text-on-surface transition-colors" aria-label={t('chat.share')}><Share2 size={17} /></button>
          <button className="focus-ring w-9 h-9 flex items-center justify-center rounded-lg text-on-muted hover:bg-surface-4 hover:text-on-surface transition-colors" aria-label={t('chat.moreOptions')}><MoreVertical size={18} /></button>
        </div>
      </div>

      {/* Messages */}
      <div className="flex-1 min-h-0 overflow-y-auto py-6" ref={scrollRef}>
        ...
        <div className="max-w-[800px] mx-auto flex flex-col gap-2">
          {showUpload && (
            <div className="card-base p-4 animate-fade-up">
              <UploadBox compact onUploaded={(f) => setAttachments((prev) => [...prev, f])} />
            </div>
          )}
          ...
          <div className="glass flex items-center gap-1 px-3 py-2 rounded-2xl border border-border-md transition-all focus-within:border-primary focus-within:shadow-glow-sm">
            <button className={`focus-ring w-9 h-9 flex items-center justify-center rounded-lg transition-colors flex-shrink-0 ${showUpload ? 'bg-primary-dim text-primary' : 'text-on-muted hover:bg-surface-4 hover:text-on-surface'}`} onClick={() => setShowUpload((v) => !v)} aria-label={t('chat.attachFile')} aria-pressed={showUpload}><Paperclip size={18} /></button>
            <button className="focus-ring w-9 h-9 flex items-center justify-center rounded-lg text-on-muted hover:bg-surface-4 hover:text-on-surface transition-colors flex-shrink-0 disabled:opacity-60" onClick={() => imageInputRef.current?.click()} disabled={uploadingImages} aria-label={t('chat.attachImage')}>
            ...
            <button className="focus-ring w-9 h-9 flex items-center justify-center rounded-lg text-on-muted hover:bg-surface-4 hover:text-on-surface transition-colors flex-shrink-0" aria-label={t('chat.voiceInput')}><Mic size={18} /></button>
            <button className="focus-ring w-9 h-9 flex items-center justify-center rounded-lg text-on-muted hover:bg-surface-4 hover:text-on-surface transition-colors flex-shrink-0" aria-label={t('chat.aiTools')}><Sparkles size={18} /></button>
